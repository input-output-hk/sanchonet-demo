flake: {
  perSystem = {
    ...
    inherit (flake.config.flake) cardanoLib;
    ...
    in ''
      ...
      ENVS=(${pkgs.lib.escapeShellArgs (builtins.attrNames cardanoLib.environments)})
      for ENV in "''${ENVS[@]}"; do
  ...
  in {
    packages.run-cardano-node = pkgs.writeShellApplication {
      ...
      runtimeInputs = with config.packages; [cardano-cli cardano-node];
      text = ''
